...
export async function rollback(targetRef: string | undefined, options: { instance?: string }): Promise<void> {
  try {
    ...
    await applyTarget(reservation.target, { pendingOperation: { pending: reservation.pending, id: reservation.id }, rolledBackFrom: current.at });
...
function assertRollbackHistory(target: GroveTarget) {
  const instance = target.instance!;
  const current = currentApplied(instance);
  const previous = instance.history[1];
  if (!current || !previous) throw new Error(`${targetName(target)} has only ${instance.history.length} recorded revision${instance.history.length === 1 ? "" : "s"}; rollback needs at least two`);
